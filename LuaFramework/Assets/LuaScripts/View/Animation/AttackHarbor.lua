...
module("LuaScript.View.Animation.AttackHarbor")
...
function Step3()
	...
	-- for k,enemy in pairs(enemyList) do
		-- function func()
			...
			-- local cfg_harbor = CFG_harbor[212]
			-- local toShip = {id=212, hp=999, x=cfg_harbor.showX, y=cfg_harbor.showY}
			-- local startTime = os.oldClock
			
			-- local timer = mTimer.SetInterval(nil, math.random()*0.5+1.8)
			-- local AttackFunc = function()
				-- local toCsShip = mHarborManager.GetCsHarbor(212)
				-- if toCsShip and oCsShip.activeSelf and fromCsShip.activeSelf then
	...
	-- local AttackFunc = function()
		...
		-- local fromCsShip = mHarborManager.GetCsHarbor(212)
		...
		-- if enemy then
			...
			-- if fromCsShip and toCsShip.activeSelf and fromCsShip.activeSelf then
				-- mBulletManager.InitBullet(8, 4, 1, 300+math.random(50), 500, fromCsShip, toCsShip, fromShip, toShip)
			-- end
		-- end
	-- end
