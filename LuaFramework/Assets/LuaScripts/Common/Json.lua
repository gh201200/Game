...
function Json.decode_scanArray(s,startPos)  
  ...
  assert(string.sub(s,startPos,startPos)=='[','Json.decode_scanArray called but array does not start at position ' .. startPos .. ' in string:\n'..s )  
  startPos = startPos + 1  
...
function Json.decode_scanComment(s, startPos)  
  assert( string.sub(s,startPos,startPos+1)=='/*', "Json.decode_scanComment called but comment does not start at position " .. startPos)  
...
function Json.decode_scanObject(s,startPos)  
  ...
  local stringLen = string.len(s)  
  local key, value  
  assert(string.sub(s,startPos,startPos)=='{','Json.decode_scanObject called but object does not start at position ' .. startPos .. ' in string:\n' .. s)  
  startPos = startPos + 1  
  repeat  
    startPos = Json.decode_scanWhitespace(s,startPos)  
...
-- START SoniEx2  
-- Initialize some things used by Json.decode_scanString  
...
function Json.decode_scanString(s,startPos)  
  assert(startPos, 'Json.decode_scanString(..) called without start position')  
  local startChar = string.sub(s,startPos,startPos)  
  -- START SoniEx2  
  -- PS: I don't think single quotes are valid JSON  
  assert(startChar == [["]] or startChar == [[']],'Json.decode_scanString called for a non-string')  
